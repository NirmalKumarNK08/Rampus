    ...
    <div>
      <div className='m-w-full relative z-30 h-1 md:h-auto flex justify-center'>
         ...
         <div className={`fixed top-0 right-0 ${menuOpen ? "flex" : "hidden"} transition-all duration-500 ease-in-out flex flex-col pt-24 px-8 w-52 gap-7 text-lg h-full bg-gray-700`}>
            <Link onClick={() => setMenuOpen(false)} className={links} to="/"><IoHome /> Home</Link>
            <Link onClick={() => setMenuOpen(false)} className={links} to="/sponsors"><FaHandshake /> Sponsors</Link>
            <Link onClick={() => setMenuOpen(false)} className={links} to="/team"><RiTeamFill /> Team</Link>
            <Link onClick={() => setMenuOpen(false)} className={links} to="/house"><GiCaptainHatProfile /> House</Link>
            <Link onClick={() => setMenuOpen(false)} className={links} to="/events"><MdEmojiEvents /> Events</Link>
            <Link onClick={() => setMenuOpen(false)} className={links} to="/about"><FaExclamationCircle /> About</Link>
            <Link onClick={() => setMenuOpen(false)} className={links} to="/contact"><MdConnectWithoutContact /> Contact</Link>
         </div>
      </div>
